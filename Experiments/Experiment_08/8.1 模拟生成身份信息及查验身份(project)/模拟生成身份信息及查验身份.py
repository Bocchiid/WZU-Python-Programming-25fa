...
def id17_to_18(id_number):
    ...
    sum_up = sum([int(id_number[i]) * ls[i] for i in range(17)])

    return id_number + ecc[sum_up % 11]
    #####################End#####################################
...
def all_of_nation(nation_file):
    ...
    return random.choice(text.split())[:-1]
    #####################End#####################################
...
def judge(txt):
    ...
    if txt == '姓名':
        ...
        print('持证人居住于' + area_of_live(address_and_code[1], area_number))
    else:
        print('输入错误')
